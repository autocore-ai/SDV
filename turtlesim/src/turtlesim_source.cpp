...
void TurtleSimSource::goalResponseCallback(std::shared_future<rclcpp_action::ClientGoalHandle<turtlesim::action::RotateAbsolute>::SharedPtr> future)
{
  RCLCPP_DEBUG(nh_->get_logger(), "Goal response received");
  this->goal_handle_ = future.get();
}
...
int main(int argc, char** argv)
{
  ...
  TurtleSimSource source;

  signal(SIGINT,quit);

  int rc = source.keyLoop();
  input.shutdown();
  rclcpp::shutdown();
